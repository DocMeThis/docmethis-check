...
"""Unit tests for the docmethis-check pre-push contract guard."""
...
def test_contract_pattern_matches_exact_path_and_directory_prefix(guard_module: ModuleType) -> None:
    ...
    patterns = ["pyproject.toml", "tests/check/"]

    assert guard_module._is_contractual("pyproject.toml", patterns)
    assert guard_module._is_contractual("tests/check/test_check_c8.py", patterns)
    assert not guard_module._is_contractual("pyproject.toml.bak", patterns)
    assert not guard_module._is_contractual("tests/check.py", patterns)
